...
@section('subtitle', 'Daftar Pendidik Sekolah')
...
<div class="max-w-5xl mx-auto">
    ...
    <div class="mb-6">
        ...
        <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Daftar guru aktif di lingkungan sekolah. Pengelolaan data guru hanya dapat dilakukan melalui portal utama Poros Data.
        </p>
    </div>
    ...
    <div class="mb-6 p-4 rounded-2xl bg-amber-50 dark:bg-amber-950/20 border border-amber-100 dark:border-amber-900/30 flex items-start gap-3">
        ...
        <div>
            ...
            <span class="block text-xs text-amber-650 dark:text-amber-455 mt-0.5">Data guru tersinkronisasi langsung dari database Poros Data. Hubungi administrator utama untuk melakukan perubahan data.</span>
        </div>
    </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-100/50 dark:bg-slate-900/60 border-b border-slate-200/80 dark:border-slate-800/80">
                        ...
                        <th class="px-6 py-4.5 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Nama Lengkap</th>
                        <th class="px-6 py-4.5 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Kode DUK (Username)</th>
                ...
                <tbody class="divide-y divide-slate-200/60 dark:divide-slate-800/60">
                    @forelse($teachers as $index => $teacher)
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-900/30 transition-colors">
                            ...
                            <!-- Kode DUK (Username) -->
                            <td class="px-6 py-4 text-sm font-semibold text-slate-650 dark:text-slate-350 whitespace-nowrap font-mono">
                                {{ $teacher->duk ?? $teacher->username }}
                            </td>
                            <!-- Aksi -->
