...
<head>
    ...
    <meta name="description" content="Halaman Login Portal PKL - PorosData.">
    <title>Login Portal PKL - PorosData</title>
...
<body class="min-h-screen text-slate-900 dark:text-slate-100 antialiased transition-colors duration-300 flex items-center justify-center p-4 relative bg-slate-50 dark:bg-slate-950">
    ...
    <div class="fixed inset-0 overflow-hidden pointer-events-none -z-20">
        ...
        <div class="absolute top-1/4 left-1/4 w-72 h-72 sm:w-96 sm:h-96 bg-orange-500/20 dark:bg-orange-500/10 rounded-full blur-3xl animate-pulse"></div>
        <div class="absolute bottom-1/4 right-1/4 w-72 h-72 sm:w-96 sm:h-96 bg-amber-500/20 dark:bg-amber-500/10 rounded-full blur-3xl animate-pulse" style="animation-delay: 2s;"></div>
    </div>
    ...
    <div class="w-full max-w-[420px] relative">
        <div class="glass-panel border border-slate-200/80 dark:border-slate-800/80 shadow-2xl rounded-3xl p-8 sm:p-10 transition-all duration-300 animate-fade-in">
            ...
            <div class="flex flex-col items-center mb-6">
                ...
                <p class="text-slate-400 dark:text-slate-500 text-sm font-semibold mt-1">
                    Tahun Ajaran 2026/2027
                </p>
            </div>

            <!-- Login Form -->
            <form action="{{ url('/porosdata/portal-pkl/login') }}" method="POST" class="space-y-5">
                @csrf

                <!-- Username Input -->
                <div class="space-y-2">
                    <label for="username" class="text-sm font-semibold text-slate-600 dark:text-slate-300 block">
                        Username
                    </label>
                    <div class="relative rounded-2xl">
                        ...
                        <input type="text" name="username" id="username" value="{{ old('username') }}" required autofocus
                            class="block w-full pl-11 pr-4 py-3 bg-white dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800/80 focus:border-orange-500 focus:bg-white dark:focus:bg-slate-950 rounded-xl text-sm transition-all focus:outline-none focus:ring-4 focus:ring-orange-500/10 text-slate-900 dark:text-white @error('username') border-rose-500/80 focus:border-rose-500 focus:ring-rose-500/10 @enderror"
                ...
                <!-- Submit Button -->
                <button type="submit"
                    class="w-full flex items-center justify-center gap-2 mt-2 px-5 py-3 text-sm font-semibold rounded-xl bg-orange-600 hover:bg-orange-500 dark:bg-orange-700 dark:hover:bg-orange-600 text-white shadow-lg shadow-orange-500/15 dark:shadow-none hover:shadow-xl hover:shadow-orange-500/25 active:scale-[0.98] transition-all focus:outline-none focus:ring-4 focus:ring-orange-500/25 cursor-pointer">
                    Masuk <i class="fa-solid fa-sign-in-alt text-xs ml-1"></i>
                </button>
            </form>
        </div>
    </div>
